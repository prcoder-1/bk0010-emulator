...
- **Граф кода и горячие точки** — граф потока управления (переходы — дугами) и
  список самых «горячих» инструкций. Исполненный код свёрнут в **подпрограммы**
  (маркер `▶`, число инструкций `{N}`); **левый клик** разворачивает подпрограмму
  в её **базовые блоки** `[N]`, а блок — в отдельные инструкции (повторный клик
  сворачивает; заодно ведёт дизассемблер на адрес). **Правый клик** скрывает
  инструкцию из графа/списка, `Del` — сброс (показать всё и всё свернуть). Граф
  можно **скроллировать** (колесо / перетаскивание / стрелки) и **масштабировать**
  (Ctrl+колесо или `+`/`−`, авто-фокус — `Home`).
