...
- **Граф кода и горячие точки** — граф потока управления (исполненный код
  сгруппирован в **базовые блоки**, переходы — дугами) и список самых «горячих»
  инструкций. Блоки по умолчанию свёрнуты (маркер `▶` и число инструкций `[N]`);
  **левый клик** разворачивает/сворачивает блок и ведёт дизассемблер на его адрес,
  **правый клик** скрывает инструкцию из графа/списка (`Del` — вернуть все). Граф
