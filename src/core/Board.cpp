...
bool Board::loadBin(const std::string& path, bool run, uint16_t* outAddr, uint16_t* outLen) {
    ...
    if (run) {
        ...
        // стек в области 0760..0776 (trap кладёт PSW+PC, дальше вложенные JSR). Такая
        // игра грузится в 0760 и заливает весь этот диапазон копиями своего стартового
        // адреса — тогда финальный RTS/RTI монитора снимает со стека не адрес возврата,
        // а этот адрес, и «возврат» уходит прямо в игру. Формат .BIN хранить стартовый
        // адрес отдельно не умеет, а мы грузим прямым poke (без EMT 36), поэтому берём
        // его сами: первое слово слота (весь диапазон залит одним значением).
        // Примеры: VALLEY (→01000), БАТИСКАФ (→036576).
        uint16_t start = addr;
        if (addr < 01000) {
