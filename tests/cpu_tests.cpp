...
int main() {
    ...
    // ---- Длительность кадра: 61440 тактов (48,83 Гц), а не 60000 ----
    {
        ...
        b.cpu().reset(01000, 0340);
        b.runTicks(b.ticksPerFrame());
        CHECK(b.vp037().scanline() >= 319 && b.vp037().scanline() <= 320,
              "за кадр развёртка проходит ровно 320 строк");
    }
    ...
    }
    {
        ...
        b.cpu().reset(01000, 0340);
        b.runFrame();
        CHECK(b.vp037().scanline() >= 150, "за кадр луч проходит основную часть строк");
    }
